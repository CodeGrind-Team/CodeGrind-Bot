...
class NotificationTypeSelect(discord.ui.Select):
    def __init__(self, selected_options: List[discord.SelectOption], available_types: List[str]):
        ...
        self.label_to_type = {"maintenance": "maintenance", "daily question": "daily_question", "leaderboard winners": "winners"}

        options = get_options(available_types)

        super().__init__(placeholder="Select notification types", max_values=len(options), min_values=1, options=options)
